...
export function analyseSession(args: AnalysisArgs): SessionAnalysis {
  ...
  // -------------------------------------------------------------------------
  //
  // Räknas bara på servern, i `metabolic-profile.ts`. Modellen och
  // referensdatan ska inte följa med till webbläsaren – det här är den del av
  // appen folk betalar för. Den här funktionen körs också i klienten, så här
  // står bara att svaret finns någon annanstans.
  if (protocol === "metabol-profil") {
    return {
      metrics: [],
      zones: [],
      zoneUnit: "W",
      warnings: ["Metabol profil räknas på servern."],
    };
  }
